...
<div class="container mx-auto px-4 pt-16">
    ...
    <p class="text-orange-500">You have currently <b>{{ $howManyMovies }}</b> watched so many movies.</p>
        ...
        <div class="MyCinemaList-Search"> <!-- MyCinemaList-Search -->
            <h2 class="uppercase tracking-wider text-orange-500 text-lg font-semibold text-center">SEARCH MOVIE IN YOUR COLLECTION</h2>
                @livewire('movie-search')
        </div> <!-- End-MyCinemaList-Search -->
        ...
        <div class="MyCinemalist-movies"> <!-- MyMovieList-movies -->
            ...
            <div class="movies grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
        ...
        <h2 class="uppercase tracking-wider text-orange-500 text-lg font-semibold text-center">ALL TVSHOWS</h2>
            <div class="tvshows grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
            @foreach ($userTvShows as $tvshow)
            <div class="tvshow mt-8">
                <a href="{{ route('tv.show', $tvshow['tvshow_id']) }}">
                    <img src="https://image.tmdb.org/t/p/w500{{ $tvshow['img'] }}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150">
                </a>
                <div class="mt-2">
...
<script>
    ...
    let elem = document.querySelector('.movies');
    let infScroll = new InfiniteScroll( elem, {
        path: '/collections/2?page=@{{#}}&sortBy={{$sortBy}}',
        append: '.movie',
        status: '.page-load-status'
    });
    let elem1 = document.querySelector('.tvshows');
    let infScroll1 = new InfiniteScroll( elem1, {
        path: '/collections/2?page=@{{#}}&sortBy={{$sortBy}}',
        append: '.tvshow',
        status: '.page-load-status'
    });
